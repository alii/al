...
use super::{Diagnostic, Severity, count_errors, count_warnings};
...
pub fn print_diagnostics(diagnostics: &[Diagnostic], source: &str, file_path: &str) {
    ...
    if error_count > 0 || warning_count > 0 {
        ...
        if error_count > 0 {
            ...
            parts.push(format!(
                "{COLOR_BOLD}{COLOR_RED}{error_count} {noun}{COLOR_RESET}"
            ));
        }
        ...
        if warning_count > 0 {
            ...
            parts.push(format!(
                "{COLOR_BOLD}{COLOR_YELLOW}{warning_count} {noun}{COLOR_RESET}"
            ));
        }

        output.push(format!("Found {}", parts.join(" and ")));
    }

...
/// Encode the low `num_bits` of `value` MSB-first into a fresh bit-string.
/// Bits beyond 64 are zero. Total: never panics, never errors.
pub fn from_int(value: i64, num_bits: u64) -> Vec<u8> {
    ...
    // Left-align the low `num_bits`, leaving `tail` pad bits in the last byte.
    // The shifted value spans at most 71 bits, so 16 bytes of a u128 cover it.
...
/// matching the language's wrapping integer arithmetic. Total.
pub fn read_int(bytes: &[u8], end: u64, at: u64, num_bits: u64) -> i64 {
    // Fast path: byte-aligned start, width fitting one word. `bytes` may be a
    // shared backing extending past `end`, the view's logical end, so anything
    // past `end` must read as zero rather than be copied through.
    ...
    // Misaligned fallback, one bit per iteration. Only the low 64 bits survive
    // the `as i64`, so the loop is capped and a hostile width cannot spin.
...
/// Decode one UTF-8 codepoint starting at bit `at`, as `(codepoint,
/// bits_consumed)`. `None` on too few bits or invalid UTF-8. Total.
pub fn read_utf8(bytes: &[u8], end: u64, at: u64) -> Option<(u32, u64)> {
    ...
    // The lead byte fixed the length, so a valid `buf[..n]` is one codepoint.
...
mod tests {
    ...
    fn read_int_aligned_word_widths() {
        ...
        // Reading past `end` zero-fills even though the backing slice has live
        // bytes there.
    ...
    fn read_utf8_unaligned() {
        ...
        // Shift 4 bits so every codepoint starts misaligned.
        let bits = 4 + s_bits;

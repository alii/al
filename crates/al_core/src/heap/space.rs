...
use super::sizing::{MAX_OLD_SEGMENT_WORDS, OLD_SEGMENT_MIN_WORDS};
...
impl Space {
    ...
    pub(super) fn bump(&mut self, len: usize) -> *mut u64 {
        assert!(
            len <= self.available(),
...
pub(super) struct SegSpace {
    ...
    /// Capacity for the next appended segment (doubles, clamped).
    next_words: usize,
}

impl SegSpace {
    pub(super) fn new(initial_segment_words: usize) -> SegSpace {
        SegSpace {
            segments: Vec::new(),
            next_words: initial_segment_words,
        }
    }
    ...
    pub(super) fn bump(&mut self, words: usize) -> *mut u64 {
        ...
        if need_new {
            ...
            self.next_words = self.next_words.saturating_mul(2).min(MAX_OLD_SEGMENT_WORDS);
        }
    ...
    pub(super) fn clear(&mut self) {
        self.segments.clear();
        self.next_words = OLD_SEGMENT_MIN_WORDS;
    }
}

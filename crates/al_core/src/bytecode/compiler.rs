...
impl IncrementalSession {
    ...
    pub fn prepare_rename(
        ...
        let id = self.graph.def_id_at(m, line, col)?;
        let cursor = self
            .graph
            .module_refs(m)
            .and_then(|mr| {
    ...
    fn symbol_of(&self, d: &Definition) -> Option<SymbolInfo> {
        ...
        Some(SymbolInfo {
            name: d.name.clone(),
            kind: d.entity(),
            module: self.graph.module_path(d.defid.module)?.clone(),
            span: d.span(),
            container: None,
        })
    }

    /// goto-definition: resolve the occurrence (or declaration) under the
    /// cursor to its canonical declaration's `(module, span)`, following the
    /// occurrence to its `target` (possibly cross-module).
    pub fn definition(
        &self,
        module_or_uri: &str,
        line: i32,
        col: i32,
    ) -> Option<(ModulePath, Span)> {
        let m = self.module_for(module_or_uri)?;
        // Chain through an import alias (`{X as Y}`) to X's real declaration;
        // `prepare_rename` deliberately keeps the raw alias `DefId`.
        let id = self.graph.canonical(self.graph.def_id_at(m, line, col)?);
        Some((self.graph.module_path(id.module)?.clone(), id.span))
    }

    /// find-references: the declaration plus every occurrence workspace-wide.
    /// The declaration's own `Definition` self-occurrence is de-duplicated
    /// against the explicit declaration entry so a name is not listed twice.
    pub fn references(&self, def: DefId) -> Vec<(ModulePath, Span)> {
        let mut out: Vec<(ModulePath, Span)> = Vec::new();
        if let Some(p) = self.graph.module_path(def.module) {
            out.push((p.clone(), def.span));
        }
        for rr in self.graph.references_to(def) {
            if (rr.module, rr.span.start_line, rr.span.start_column)
                == (def.module, def.span.start_line, def.span.start_column)
            {
                continue;
            }
            if let Some(p) = self.graph.module_path(rr.module) {
